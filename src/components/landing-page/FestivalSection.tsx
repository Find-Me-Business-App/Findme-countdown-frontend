                    ...
                    >
                        ...
                        {"LAUNCH".split("").map((letter, i) => (
                            <motion.span
                                ...
                                custom={i + 7}
                                variants={letterVariants}
                                className={`${getLetterColor(letter, i, "LAUNCH")} filter drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] inline-block`}
                            >
                                {letter}
                            </motion.span>
                        ))}
...
function getLetterColor(letter: string, index: number, word: string) {
    if (word === "FINDME") {
        // "FIND" (indices 0-3) should be #2B365A
        // "ME" (indices 4-5) should be #FF3D00
        return index < 4 ? "text-[#2B365A]" : "text-[#FF3D00]";
    }

    if (word === "LAUNCH") {
        // Colors from user gradient: FF3D00 42A5F5 823E8D EC9D03 06CA02
        const launchPalette = [
            "text-[#FF3D00]", // Red
            ...
            "text-[#EC9D03]", // Amber
            "text-[#06CA02]", // Green
            "text-[#06CA02]"  // Repeat Green for 6th letter 'H'
        ];
        return launchPalette[index % launchPalette.length];
    }

    if (word === "FESTIVAL") {
        // Reusing the general palette from before for Festival
