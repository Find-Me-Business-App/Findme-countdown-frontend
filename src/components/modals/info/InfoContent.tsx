    ...
    return (
        <>
            ...
            <div className="relative z-10 flex flex-col items-center md:items-start text-center md:text-left md:p-14 w-full md:w-[65%] whitespace-normal overflow-y-auto scrollbar-hide max-h-[50vh] md:max-h-[70vh]">
                ...
                <InfoBody title={config.title} description={config.description} titleColor={config.titleColor} />

                {/* Sub Content */}
                <InfoFooter subDescription={config.subDescription} />
            </div>
        </>
    );
}
...
function InfoBody({ title, description, titleColor }: { title: string; description: string; titleColor?: string }) {
    return (
        <div className="mb-8 md:mb-10 flex flex-col items-center md:items-start">
            <h2 className={`text-xl md:text-3xl font-bold mb-4 ${titleColor || 'text-white'}`}>
                {title}
            </h2>
...
function InfoFooter({ subDescription }: { subDescription: string }) {
    return (
